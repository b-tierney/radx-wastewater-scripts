...
c = read.csv('db_counts.csv')
ggplot(data = c, aes(x = reorder(database,count),y=count)) + xlab('') +geom_bar(stat='identity') + theme_bw() + theme(axis.text.x = element_text(angle = 45,hjust=1)) + ggtitle('AMRs identified by database')
ggsave('~/Dropbox (Mason Lab)/radx/plots/amrs_by_database_card.pdf',width=8,height=4)

data = read.delim('abricate_card.tab',check.names=F) %>% mutate(sample_id = strsplit(`#FILE`,'/') %>% map_chr(2) %>% gsub('_rnaviral_assembled','',.))
...
# filter out AMRs with a prev of 1 
...
temp = rowSums(data_wide_hm) %>% data.frame %>% rownames_to_column('sample_id') %>% dplyr::rename(`AMR Count` = ".")
data_wide_anno = inner_join(data_wide_anno,temp)%>% column_to_rownames('sample_id') %>% mutate(`AMRs/10M reads` = `AMR Count`/(V2/10000000))
data_wide_anno=data_wide_anno[rownames(data_wide_hm),]
row_ha = rowAnnotation(`AMRs/10M reads` = data_wide_anno$`AMR Count`,LOCATION_TYPE = data_wide_anno$LOCATION_TYPE)
...
pdf('~/Dropbox (Mason Lab)/radx/plots/amr_heatmap_card.pdf',width=14,height=10)
a #%v% b
dev.off()
...
ggsave(plot=plot,'~/Dropbox (Mason Lab)/radx/plots/amr_prev_card.pdf',width=5,height=2)
...
temp = bind_rows(temp,a,b) %>% distinct 
data_wide_rescom = inner_join(melt(res%>% dplyr::rename(sample_id  = variable) ) %>% distinct ,temp)
rescom = data_wide_rescom %>% group_by(LOCATION_TYPE,variable) %>% summarise(Freq = sum(value)) %>% arrange(desc(Freq))
rescom2 = rescom %>% group_by(variable) %>% summarise(Freq=sum(Freq)) %>% arrange(desc(Freq)) %>% mutate(RESISTANCE=as.character(variable))
rescom$variable=factor(rescom$variable,levels=(rescom2$variable))
rescom = inner_join(rescom,rescom %>% ungroup %>% group_by(variable) %>% mutate(total = sum(Freq)) %>% select(-Freq)) %>% mutate(prop = Freq/total)

plot = ggplot(rescom,aes(x = prop,y=variable,fill=LOCATION_TYPE)) + geom_bar(stat='identity',width=.9) + theme_bw() + ylab('') + xlab('Prevalence') + ylab('') + scale_fill_manual(values=plae) + geom_text(aes(label = total, x = 1 + 0.1,size=8, y = variable), size = 3, color = "black") + coord_flip() + theme(legend.position = 'none',axis.text.x = element_text(angle=45,hjust =1))

ggsave(plot=plot,'~/Dropbox (Mason Lab)/radx/plots/amr_mechprev_card.pdf',width=5,height=2)
...
ggplot(data_wide_sum,aes(x = LOCATION_TYPE,y=prop)) + geom_quasirandom(size=3) + theme_bw() + xlab('') + ggtitle('AMR identification by site') + ylab('AMRs/10M reads') + stat_compare_means(method = 'wilcox.test',comparisons = list(c('Hospital/Medical Campus','Dormitory'),c('Hospital/Medical Campus','University Campus'),c('Hospital/Medical Campus','Wastewater Treatment Plant'))) + theme(axis.text.x = element_text(angle = 45,hjust=1))
ggsave('~/Dropbox (Mason Lab)/radx/plots/locrich_card.pdf',width=3.5,height=8)
dev.off()

# amr over time averaged
...
ggplot(data_wide_sum_month,aes(x = month2,y=count,color=LOCATION_TYPE))+ geom_quasirandom(size=3) + theme_bw() + ylab('AMRs/10M reads') + ggtitle('AMR richness over time')+xlab('') + scale_color_manual(values=plae) + theme(legend.position ="None")
ggsave('~/Dropbox (Mason Lab)/radx/plots/amrs_over_time_card.pdf',width=5,height=4)
